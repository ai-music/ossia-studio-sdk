...
export class BackingTrackEndpoint {
   ...
   * @param payload
   */
  public async create(payload: IBackingTrackCreate & IBackingTrackAudioFiles): ApiResponse<IBackingTrack> {
    const { originalBackingTrackAudio, previewBackingTrackAudio, ...rest } = payload
    const response = this.client.post<IBackingTrackCreate, IBackingTrack>(this.path, rest)
    const { data } = await response
    await uploadFileWithAwsPolicy(data.uploadPolicy, originalBackingTrackAudio, `backing_tracks/${data.id}/original.wav`)
    await uploadFileWithAwsPolicy(data.uploadPolicy, previewBackingTrackAudio, `backing_tracks/${data.id}/preview.mp3`)
    return response
  }
